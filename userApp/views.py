...
from socialApp.models import Favorite
...
def signup(request):
	if request.user.is_authenticated: 
		...
		if request.method == 'GET':
			return render(request, 'userApp/sign_up.html', {'form': UserCreationForm})
		else:
			if not(request.POST['username']):
				...
				try:
					...
					return render(request, 'userApp/sign_up.html', {'form': UserCreationForm(), 'error_contex': error_contex})	
			return render(request, 'userApp/sign_up.html', {'form': UserCreationForm(), 'error_contex': error_contex})
	...
	if profile.about:
		if request.method == 'POST':
			...
			if int(request.POST['age']) < 18:
				return render(request, 'userApp/user_account.html', {'error':'Your age must be at least 18 years old'})
			elif not str(request.POST['age']).isnumeric():
				return render(request, 'userApp/user_account.html', {'error':'Uncorrect age field'})
			else:
				try:
					u_form.save()
					p_form.save()
					return redirect('userApp:user_account') # Перенаправление на страницу профиля пользователя
				except ValueError:
					return render(request, 'userApp/user_account.html', {'error':'Files is too large, requirement is less than 2.5 MB'})
		...
		return render(request, 'userApp/user_account.html', context)
...
def sign_up_step_one(request):
	if request.method == 'POST':
		...
		if not(request.POST['first_name']):
			return render(request, 'userApp/sign_up_step_one.html', {'error': 'First name can\'t be empty'})
		if not(str(request.POST['first_name']).isalpha()):
			return render(request, 'userApp/sign_up_step_one.html', {'error': 'First name can\'t have numbers'})
		elif not(request.POST['last_name']):
			return render(request, 'userApp/sign_up_step_one.html', {'error': 'Last name can\'t be empty'})
		if not(str(request.POST['last_name']).isalpha()):
			return render(request, 'userApp/sign_up_step_one.html', {'error': 'Last name can\'t have numbers'})
		elif not(request.POST['age']):
			return render(request, 'userApp/sign_up_step_one.html', {'error': 'Age can\'t be empty'})
		elif int(request.POST['age']) < 18:
			return render(request, 'userApp/sign_up_step_one.html', {'error': 'Your age must be at least 18 years old'})
		elif not str(request.POST['age']).isnumeric():
				return render(request, 'userApp/sign_up_step_one.html', {'error':'Uncorrect age field'})
		else:
			step_one_form.save()
	...
	return render(request, 'userApp/sign_up_step_one.html', context)
...
def sign_up_step_two(request):
	...
	if profile.first_name and profile.last_name and profile.age:
		...
		return render(request, 'userApp/sign_up_step_two.html', context)
	return redirect('userApp:sign_up_step_one')
	...
	if profile.city and profile.sex and profile.seeking:
		if request.method == 'POST':
			...
			if not(request.POST['about']):
				return render(request, 'userApp/sign_up_step_three.html', {'error': 'About field can\'t be empty'})
			else:
				try:
					step_three_form.save()
					return redirect('dating_app:dating')
				except ValueError:
					return render(request, 'userApp/sign_up_step_three.html', {'error': 'File is too large, requirement is less than 2.5 MB'})
		...
		return render(request, 'userApp/sign_up_step_three.html', context)
	return redirect('userApp:sign_up_step_two')